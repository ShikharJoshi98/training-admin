    ...
    return (
        <div className="bg-black/50 z-60 text-white fixed inset-0 flex items-center justify-center p-2">
            <div className="bg-slate-700 max-h-[90vh] max-w-[90vw] overflow-y-auto flex flex-col w-full rounded-xl p-4 shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="px-0 sm:px-40 mt-10 flex flex-col gap-4">
                        ...
                        <p>Add Subchapter</p>
                        {
                            formValues.subChapter.map((sub, index) => (
                                <div key={index} className="flex items-center gap-2">
                                    <input value={sub} required onChange={(e) => handleSubChapterChange(index, e.target.value)} placeholder="Enter Subchapter" type="text" className="bg-black/40 rounded-md border text-white/80 w-full py-1 px-2" />
